...
def plot_one_box(x, img, color=None, label=None, line_thickness=3):
    ...
    if label:
        ...
        # 将下面的注释打开，注释掉上面一句话，可以在预测框上面显示时间
        # clock = time.strftime('%H:%M:%S', time.localtime(time.time()))  # 打印当前时间
        # l_c = ("%s,%s"%(label,clock))  # 创建时间和标签的文本
        # cv2.putText(img,l_c, (c1[0], c1[1] - 2),
